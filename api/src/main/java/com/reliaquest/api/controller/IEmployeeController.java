...
public interface IEmployeeController<Entity, Input> {
    ...
    @PostMapping()
    ResponseEntity<Entity> createEmployee(@RequestBody EmployeeRequest employeeInput);

    @DeleteMapping("/{id}")
    ResponseEntity<String> deleteEmployeeById(@PathVariable String id);

    @DeleteMapping("/name/{employeeName}")
    ResponseEntity<Boolean> deleteEmployeeByName(@PathVariable String employeeName);
